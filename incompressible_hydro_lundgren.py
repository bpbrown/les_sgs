...
problem.substitutions['divu'] = 'abs(dx(u)+dy(v)+dz(w))'
problem.substitutions['enstrophy'] = 'O_x*O_x+O_y*O_y+O_z*O_z'
problem.substitutions['vol_avg(A)']   = 'integ(A)/Lx/Ly/Lz'
problem.add_equation('dt(u) + dx(p) - R*Lap(u) - Q*u = -u_grad(u)')
problem.add_equation('dt(v) + dy(p) - R*Lap(v) - Q*v = -u_grad(v)')
problem.add_equation('dt(w) + dz(p) - R*Lap(w) - Q*w = -u_grad(w)')
...
scalar.add_task('vol_avg(divu)', name='divu')

# CFL
CFL = flow_tools.CFL(solver, initial_dt=dt, cadence=cfl_cadence, safety=0.2,
...
flow.add_property('divu', name='divu')
...
try:
    ...
    while solver.proceed and good_solution:
        ...
        if solver.iteration == 1 or (solver.iteration) % report_cadence == 0:
            log_string = 'Iteration: {:d}, Time: {:g}, dt: {:5.2g}, Re = {:.3g}, Ens = {:.3g}, |divu|={:.3g}'.format(
                solver.iteration, solver.sim_time, dt, flow.volume_average('Re'), flow.volume_average('enstrophy'), flow.max('divu'))
